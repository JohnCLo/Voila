   ...
   setTimeout(function () {    
      ...
      request(request_url, function (error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
          ...
          if(diffDays < 10){ // make sure there is complete data avaliable, despite successful request
            ...
            for (var i = 0; i < periods.length; i++) {
              ...
              for (var j = 0; j < body.data.length; j++) {
                if (body.data[j][0] >= periods[i][0] && body.data[j][0] <= periods[i][1]){ //error: second body.data[i][0]
              ...
              //console.log(body.code, data[data.length - 1]);
              // orginally 10
              var latestReturn_pct = ((data[data.length - 1][1] / data[data.length - 5][1]) - 1) * 100; // note this is run for every period, but code below only takes last calculated period
                  latestReturn_pct = latestReturn_pct.toFixed(2)
                  if(latestReturn_pct > 0){latestReturn_pct = '+'+latestReturn_pct;}

              results.push({'slope': slope, 'r_squared': r_squared, 'period': periods[i], 'latestReturn_pct': latestReturn_pct + '%'});
            };

            collection.push({"ticker": body.code, 'results': results});
            ...
            // perform batch analysis for all stocks here ///////////////////
            if(body.code == last_stock){
              var analysis = [];
              ...
              for (var i = 0; i < analysis.length; i++) {
                analysis_score.push({'ticker': analysis[i].ticker,'latestReturn_pct':analysis[i].results.latestReturn_pct, 'score':ss.mean(analysis[i].ranks)})
              };
              ...
              console.log(analysis_score_sorted);
