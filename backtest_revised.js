...
for (var i = 0; i < 250; i++) { //180 // 1 years, 6 months, 2 months, //400
  ...
  request(request_url, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      for (var i = 0; i < cleaned_complete.length; i++) { // clean_complete.length should equal spy_returns.length
         ...
         if(i > 5){ // instead of using if and set length, make it relative length
          var reg_batch = [];
          for (var j = 1; j < 4; j++) {
            reg_batch.push(spy_capital_reg[spy_capital_reg.length - j])
          };
          // Important! Which way the array is arranged is important for reg analysis!
          var reg_batch_reversed = reg_batch.reverse();
          reg_batch = [];
          for (var j = 0; j < reg_batch_reversed.length; j++) {
            reg_batch.push([j*7, reg_batch_reversed[j]]) // *7 for number of days in a week
          };
          //var reg_batch = [spy_capital_reg[spy_capital_reg.length -1],spy_capital_reg[spy_capital_reg.length -2],spy_capital_reg[spy_capital_reg.length -3]]
          //console.log(reg_batch);
          //console.log(spy_capital_record);
          var spy_predictor_weight = ss.linear_regression().data(reg_batch).m();

          spy_predictor_weight = spy_predictor_weight / 100;
         } else {
          var spy_predictor_weight = 0
         }

        //console.log(spy_predictor_weight);
        var radians = Math.atan(spy_predictor_weight);
        var degrees = radians * (180/Math.PI);

        var weight = degrees / 90;

        if (weight > 0){
          var weight_a = weight
          var weight_b = 1 - weight /// !!! need to change so 0,0 is no weights for either
        } else if (weight < 0) {
          var weight_a = 1-(-weight)
          var weight_b = -weight
        } else if (weight == 0){
          var weight_a = 0;
          var weight_b = 1;
        ...
        console.log(spy_predictor_weight, degrees, weight, weight_a, weight_b);
        ...
        ////////////////////////////////
        // var trigger = 0;
        // if(spy_capital_record[spy_capital_record.length -1] < 90000){
        //   console.log("hello");
        // }
          

        //console.log(spy_capital_record[spy_capital_record.length -1]);

        //console.log(spy_capital_record[spy_capital_record.length -1], trigger);
